...
class ProcessingModule extends _TonSdkModule {
  ...
  Future<void> monitor_messages(ParamsOfMonitorMessages params) async {
    await _tonCore.request(
        'processing.monitor_messages', params.toString(), null);
  }
  ...
  Future<MonitoringQueueInfo> get_monitor_info(
    ...
    final res = await _tonCore.request(
        'processing.get_monitor_info', params.toString(), null);
  ...
  Future<ResultOfFetchNextMonitorResults> fetch_next_monitor_results(
    ...
    final res = await _tonCore.request(
        'processing.fetch_next_monitor_results', params.toString(), null);
  ...
  Future<void> cancel_monitor(ParamsOfCancelMonitor params) async {
    await _tonCore.request(
        'processing.cancel_monitor', params.toString(), null);
  }

  ///Sends specified messages to the blockchain.
  Future<ResultOfSendMessages> send_messages(
      ParamsOfSendMessages params) async {
    final res = await _tonCore.request(
        'processing.send_messages', params.toString(), null);
    return ResultOfSendMessages.fromMap(res);
  }

  ///Sends message to the network and returns the last generated shard block of the destination account
  ///before the message was sent. It will be required later for message processing.
  Future<ResultOfSendMessage> send_message(
      ParamsOfSendMessage params, Function? responseHandler) async {
    final res = await _tonCore.request(
  ...
  Future<ResultOfProcessMessage> wait_for_transaction(
      ParamsOfWaitForTransaction params, Function? responseHandler) async {
    final res = await _tonCore.request(
  ...
  Future<ResultOfProcessMessage> process_message(
      ParamsOfProcessMessage params, Function? responseHandler) async {
    final res = await _tonCore.request(
