...
class DebotModule extends _TonSdkModule {
  ...
  Future<RegisteredDebot> init(
      ParamsOfInit params, Function? responseHandler) async {
  ...
  Future<void> start(ParamsOfStart params) async {
    await _tonCore.request('debot.start', params.toString(), null);
  }

  ///Downloads DeBot from blockchain and creates and fetches its metadata.
  Future<ResultOfFetch> fetch(ParamsOfFetch params) async {
    final res = await _tonCore.request('debot.fetch', params.toString(), null);
  ...
  Future<void> execute(ParamsOfExecute params) async {
    await _tonCore.request('debot.execute', params.toString(), null);
  }

  ///Used by Debot Browser to send response on Dinterface call or from other Debots.
  Future<void> send(ParamsOfSend params) async {
    await _tonCore.request('debot.send', params.toString(), null);
  }

  ///Removes handle from Client Context and drops debot engine referenced by that handle.
  Future<void> remove(ParamsOfRemove params) async {
    await _tonCore.request('debot.remove', params.toString(), null);
  }
}
